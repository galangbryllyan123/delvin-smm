			...
			<section class="content">
			  ...
			  <div class="row">
				...
				<!-- ./col -->
                    ...
                    <div class="row">
                        ...
                        <div class="card-box table-responsive">
                            ...
                            <p class="text-muted font-13 m-b-30">
                                "Setelah deposit, data akan tersimpan disini."
                            </p>
					<?php
					if(isset($_GET['delete'])){
						$id = trim($_GET['delete']);
						$check_edit_q = mysqli_query($db, "SELECT * FROM deposit WHERE id = '$id'");
						if(mysqli_num_rows($check_edit_q)<1){
							echo '<div class="alert alert-warning">Deposit ID tidak ada!</div>';
						} else {
							$check_edit_f = mysqli_fetch_assoc($check_edit_q);
							$simpan = mysqli_query($db, "DELETE FROM deposit WHERE id = '$id'");
							if($simpan){
						...
						if(mysqli_num_rows($check_edit_q)<1){
							...
							<div class="box box-primary box-solid">
								<div class="box-header with-border">
								  <h3 class="box-title">Meng-edit deposit <?=$id;?> </h3>
								...
								<!-- /.box-header -->
								<div class="box-body">
									<form method="post" action="act.php?editdepo" id="edit">
										<div id="result"></div>
										...
										<div class="form-group">
											...
											<select name="status" id="status" class="form-control">
												...
												<option value="fraud">Gagal</option>
										...
										<div class="form-group">
											...
											<input type="text" id="approved_by" name="approved_by" class="form-control" value="<?=$check_edit_f['approved_by'];?>"/>
										</div>
					...
					<div class="box box-primary box-solid">
						<div class="box-header with-border">
						  <h3 class="box-title">Manage Deposit</h3>
						  <div class="box-tools pull-right">
							<button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
						  </div>
						  <!-- /.box-tools -->
						...
						<div class="box-body">
						  <table class="table table-bordered">
							...
							<? $q_x = mysqli_query($db, "SELECT * FROM deposit");
							if(mysqli_num_rows($q_x)<1){
								...
								while ($data = mysqli_fetch_assoc($q_x)) {
									...
									echo '<td>'.$data['jadi'].'</td>';
									echo '<td>'.$data['sender'].'</td>';
									echo '<td><a href="'.$site->url.'payments/'.$data['proof'].'">Here</a> [ '.$data['proof'].' ]</td>';
									echo '<td>'.$data['uplink'].'</td>';
