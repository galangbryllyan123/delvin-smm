                    ...
                    <div class="row">
                        ...
                        <div class="card-box table-responsive">
                            ...
                            <p class="text-muted font-13 m-b-30">
                                "Data member tersimpan disini."
                            </p>
					<?php
					if(isset($_GET['delete'])){
						$edit_username = trim($_GET['delete']);
						$check_edit_q = mysqli_query($db, "SELECT * FROM members WHERE username = '$edit_username'");
						if(mysqli_num_rows($check_edit_q)<1){
							echo '<div class="alert alert-warning">Member belum terdaftar!</div>';
						} else {
							$check_edit_f = mysqli_fetch_assoc($check_edit_q);
							$simpan = mysqli_query($db, "DELETE FROM members WHERE username = '$edit_username'");
							if($simpan){
						...
						if(mysqli_num_rows($check_edit_q)<1){
							...
							<div class="box box-primary box-solid">
								<div class="box-header with-border">
								  <h3 class="box-title">Meng-edit user <?=$edit_username;?> </h3>
								...
								<!-- /.box-header -->
								<div class="box-body">
									<form method="post" action="act.php?edit" id="edit">
										<div id="result"></div>
										<div class="form-group">
											<label>Nama: </label>
											<input type="text" name="nama" id="nama" class="form-control" value="<?=$check_edit_f['nama'];?>"/>
										</div>
										<div class="form-group">
											<label>Password: </label>
											<input type="hidden" name="username" id="username" class="form-control" value="<?=$check_edit_f['username'];?>"/>
											<input type="text" name="password" id="password" class="form-control" value="<?=$check_edit_f['password'];?>"/>
										</div>
										<div class="form-group">
											<label>Level: </label>
											<input type="text" name="level" id="level" class="form-control" value="<?=$check_edit_f['level'];?>"/>
										</div>
										<div class="form-group">
											<label>Saldo: </label>
											<input type="number" name="saldo" id="saldo" class="form-control" value="<?=$check_edit_f['saldo'];?>"/>
										</div>
										<div class="form-group">
											<label>API: </label>
											<input type="text" name="api" id="api" class="form-control" value="<?=$check_edit_f['api'];?>"/>
										</div><hr>
					...
					<div class="box box-primary box-solid">
						...
						<div class="box-body">
						  <table class="table table-bordered">
							...
							<? $q_x = mysqli_query($db, "SELECT * FROM members");
							if(mysqli_num_rows($q_x)<1){
								...
								while ($data = mysqli_fetch_assoc($q_x)) {
									...
									echo '<td>'.$data['api'].'</td>';
									echo '<td><a href="?edit='.$data['username'].'">Edit</a> / <a href="?delete='.$data['username'].'">Hapus</a></td>';
									echo '</tr>';
								}
							} ?>
						  </table>
