			  ...
			  <div class="row">
				<div class="col-md-12">
					...
					if(isset($_GET['delete'])){
						$id = trim($_GET['delete']);
						$check_edit_q = mysqli_query($db, "SELECT * FROM api_setting WHERE id = '$id'");
						if(mysqli_num_rows($check_edit_q)<1){
							echo '<div class="alert alert-warning">API ID tidak ada!</div>';
						} else {
							$check_edit_f = mysqli_fetch_assoc($check_edit_q);
							$simpan = mysqli_query($db, "DELETE FROM api_setting WHERE id = '$id'");
							if($simpan){
						...
						if(mysqli_num_rows($check_edit_q)<1){
							...
							<div class="box box-primary box-solid">
								<div class="box-header with-border">
								  <h3 class="box-title">Meng-edit api <?=$id;?> </h3>
								...
								<!-- /.box-header -->
								<div class="box-body">
									<form method="post" action="act.php?editapi" id="edit">
										<div id="result"></div>
										<input type="hidden" id="id" name="id" value="<?=$check_edit_f['id'];?>"/>
										<div class="form-control">
											<label>Provider Name:</label>
											<input type="text" name="name" id="name" class="form-control" value="<?=$check_edit_f['name'];?>"/>
										</div>
										<div class="form-control">
											<label>Provider APIKey:</label>
											<input type="text" name="apikey" id="apikey" class="form-control" value="<?=$check_edit_f['apikey'];?>"/>
										</div>
										<div class="form-control">
											<label>Provider APIUrl:</label>
											<input type="text" name="apiurl" id="apiurl" class="form-control" value="<?=$check_edit_f['apikey'];?>"/>
										</div>
					...
					<div class="box box-primary box-solid">
						...
						<div class="box-body">
							...
							<?php
							if(isset($_POST['name']) || isset($_POST['apikey']) || isset($_POST['apiurl'])){
								$name = mysqli_real_escape_string($db, trim($_POST['name']));
								$apikey = mysqli_real_escape_string($db, trim($_POST['apikey']));
								$apiurl = mysqli_real_escape_string($db, trim($_POST['apiurl']));
								if(!$name || !$apikey || !$apiurl){
									echo '<div class="alert alert-warning">No Data</div>';
								} else {
									$cek = mysqli_query($db, "SELECT * FROM api_setting WHERE apikey = '$apikey'");
									if(mysqli_num_rows($cek)<1){
										$simpan = mysqli_query($db, "INSERT INTO api_setting VALUES('', '$name', '$apikey', '$apiurl')");
										if($simpan){
										...
										echo '<div class="alert alert-warning">Maaf, apikey ini sudah terinput</div>';
									}
								}
							}
							?>
					...
					<div class="box box-primary box-solid">
						...
						<div class="box-body">
						  <table class="table table-bordered">
							...
							<? $q_x = mysqli_query($db, "SELECT * FROM api_setting");
							if(mysqli_num_rows($q_x)<1){
								...
								while ($data = mysqli_fetch_assoc($q_x)) {
									...
									echo '<td>'.$data['apikey'].'</td>';
									echo '<td>'.$data['apiurl'].'</td>';
