			  ...
			  <div class="row">
				<div class="col-md-12">
					...
					if(isset($_GET['delete'])){
						$id = trim($_GET['delete']);
						$check_edit_q = mysqli_query($db, "SELECT * FROM informasi WHERE id = '$id'");
						if(mysqli_num_rows($check_edit_q)<1){
							echo '<div class="alert alert-warning">Informasi tidak ada!</div>';
						} else {
							$check_edit_f = mysqli_fetch_assoc($check_edit_q);
							$simpan = mysqli_query($db, "DELETE FROM informasi WHERE id = '$id'");
							if($simpan){
						...
						if(mysqli_num_rows($check_edit_q)<1){
							...
							<div class="box box-primary box-solid">
								<div class="box-header with-border">
								  <h3 class="box-title">Meng-edit informasi <?=$id;?> </h3>
								...
								<!-- /.box-header -->
								<div class="box-body">
									<form method="post" action="act.php?editinfo" id="edit">
										<div id="result"></div>
										<div class="form-group">
											<label>Informasi:</label>
											<input type="hidden" name="id" id="id" value="<?=$check_edit_f['id'];?>" class="form-control"/>
											<input type="text" name="informasi" id="informasi" value="<?=$check_edit_f['informasi'];?>" class="form-control"/>
										</div>
										<div class="form-group">
											<label>Bagian:</label>
											<input type="text" name="bagian" id="bagian" value="<?=$check_edit_f['bagian'];?>" class="form-control"/>
										</div>
										<div class="form-group">
											<label>Tanggal:</label>
											<input type="text" name="date" id="date" value="<?=$check_edit_f['date'];?>" class="form-control"/>
										</div>
					...
					<div class="box box-primary box-solid">
						...
						<div class="box-body">
							...
							<?php
							if(isset($_POST['informasi']) || isset($_POST['bagian'])){
								$tanggal = date('d-m-Y');
								...
								if(!$informasi || !$bagian){
									...
									$cex = mysqli_query($db, "SELECT * FROM informasi WHERE informasi = '$informasi'");
									if(mysqli_num_rows($cex)<1){
										$simpan = mysqli_query($db, "INSERT INTO informasi VALUES('', '$informasi', '$bagian', '$tanggal')");										
										if($simpan){
											echo '<div class="alert alert-success">Sukses menambahkan!</div><script>window.location.href = "?";</script>';
										} else {
											echo '<div class="alert alert-warning">Gagal!</div>';
										}
									} else {
										echo '<div class="alert alert-warning">Maaf, informasi yang anda submit itu sudah ada!</div>';
									}
								}
							}
							?>
					...
					<div class="box box-primary box-solid">
						<div class="box-header with-border">
						  <h3 class="box-title">Manage Information</h3>
						  <div class="box-tools pull-right">
							<button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
						  </div>
						  <!-- /.box-tools -->
						...
						<div class="box-body">
						  <table class="table table-bordered">
							...
							<? $q_x = mysqli_query($db, "SELECT * FROM informasi");
